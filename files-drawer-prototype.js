(() => {
  ...
  const sheet = {
    metric: document.querySelector('[data-sheet="metric"]'), source: document.querySelector('[data-sheet="source"]'),
    numerator: document.querySelector('[data-sheet="numerator"]'), denominator: document.querySelector('[data-sheet="denominator"]'),
    period: document.querySelector('[data-sheet="period"]'), calculation: document.querySelector('[data-sheet="calculation"]')
  };
  const data = {
  ...
  function open(key) {
    ...
    drawer.classList.add('is-open'); drawer.setAttribute('aria-hidden','false'); document.body.classList.add('is-locked');
    drawer.querySelector('.close').focus();
  }
  function close() {
    drawer.classList.remove('is-open'); drawer.setAttribute('aria-hidden','true'); document.body.classList.remove('is-locked');
  ...
  files.forEach(file => file.addEventListener('click', () => { files.forEach(item => item.classList.remove('is-active')); file.classList.add('is-active'); }));
